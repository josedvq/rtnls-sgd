...
def train_loop(
    ...
    if config.segmentation_guided:
        run_name += "-segguided"
    # logger = SummaryWriter(comment=run_name)
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if config.class_conditional:
                ...
                logger.log({"loss_target_domain": loss.detach().item()})
            else:
                logs = {
                    "loss": loss.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0],
            ...
            logger.log({"loss": loss.detach().item()})
        ...
        if (
            ...
            if config.segmentation_guided:
                ...
                evaluate(config, epoch, pipeline, seg_batch, logger=logger)
            else:
                evaluate(config, epoch, pipeline, logger=logger)
